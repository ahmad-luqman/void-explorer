...
## Stone and ground material integration — 19 September 2026

The previous foreground rocks retain deliberate faceted silhouettes but read as solid-color props beside the textured slate floor. The new material reuses the prepared grit and existing slate image with per-instance physical coordinates. Fine grain, shallow filtered relief, subtle bedding and upward-face dust carry the ground's mineral treatment onto rocks. Stable identity phases avoid synchronized texture patterns and stay attached to props through recentering and native world rotation. The same GLSL function generates the WebGPU shade function.

No rock vertices, collision capsules, prop positions, terrain profiles or save records change. The existing closed formations and total triangle budget remain covered. The material adds six filtered texture reads per stone fragment and at most 16,800 bytes of per-instance attributes under the shared scenery cap. It allocates no additional image and retains image-failure fallback. Fans, succulents, crystals, minerals and relay structures keep their existing materials.

Actual views and serial frame evidence are retained in [the stone material review](benchmarks/stone-material-2026-09-19/README.md). This treatment addresses the visibly plain near rock surfaces; it does not complete shoreline composition, richer distant scene integration or physical-device acceptance. The broad pale hills at the non-coastal sites also remain an environment-art limitation, despite their working surveys and distinct landmark kits.

The first final [WebGL bay](milestones/stone-ground-loaded-bay-webgl.png), [parked-ship view](milestones/stone-ground-loaded-ship-webgl.png), [Low view](milestones/stone-ground-loaded-low-webgl.png) and [missing-image fallback](milestones/stone-ground-fallback-bay-webgl.png) retain legible rock facets with finer surface grain. Compare the same scene before this pass in the [composition milestone](milestones/composed-ground-loaded-bay-webgl.png). Viewpoint polling can stop a few meters apart; this is an actual gameplay comparison, not an identical-camera pixel benchmark.

Final [WebGPU bay](milestones/stone-ground-loaded-bay-webgpu.png), [WebGPU fallback](milestones/stone-ground-fallback-bay-webgpu.png), [Firefox](milestones/stone-ground-loaded-bay-firefox.png) and [WebKit](milestones/stone-ground-loaded-bay-webkit.png) retain the new treatment. Both seven-check production suites pass (WebGL 5.3 min, WebGPU 5.5 min); Firefox/WebKit High/Low and extended cliff-edge/return checks pass in 30.9/29.8 s. All 144 unit tests, final TypeScript/scoped lint and the static build pass. Both renderer settled samples retain 16.7 ms medians and 16.8 ms maxima. Previous moving/cold-entry outliers and broader hardware acceptance remain open.
