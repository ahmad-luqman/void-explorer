...
`LandingGear` contains a six-joint rigid-weight skeleton; `EngineCore_L_Runtime` and `EngineCore_R_Runtime` have controllable emission. Exhaust trails remain runtime effects. Static parts are merged by material for export; the saved Blender source retains the individual parts. The export contains 12 meshes, 3,932 triangles, seven materials, and no external textures or decoder dependencies.

The surface-detail pass adds segmented nacelle armor, framed service wells with lowered floors, four wing maintenance hatches, shoulder louvers and rudder seams. The surface-detail GLB reuses the original seven materials and 12 meshes. Exact bounds and all three pad centers are checked by the asset tests.

The image is a visual reference, with overlapping-view ambiguities resolved in geometry. This is an original simplified model, not a conversion of the reference game's assets. The mechanical gear pass adds three hinged legs and three counter-rotating pads. Fixed bay plates remain on the hull; the legs fold aft by 90 degrees. The Blender source has a scrubbable deployment study (frame 1 stowed, frame 49 deployed), while runtime drives the same named joints from simulation time. The current GLB is 241,408 bytes, still 12 meshes / 3,932 triangles / seven materials. Pads remain level throughout travel; deployed pad centers and ground clearance are unchanged. Fully enclosed bay doors, telescoping hydraulics and suspension compression remain outside this pass.
